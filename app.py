    ...
    def render_simulation_state(sim_type, column):
        ...
        with column:
            ...
            if sim_type == 'ai':
                ...
                if ev_cols[0].button("🚑 N", key=f"ev_n_{sim_type}"): sim.inject_emergency_vehicle('north', 'ambulance')
                if ev_cols[1].button("🚒 E", key=f"ev_e_{sim_type}"): sim.inject_emergency_vehicle('east', 'fire_truck')
                if ev_cols[2].button("🚓 S", key=f"ev_s_{sim_type}"): sim.inject_emergency_vehicle('south', 'police')
                if ev_cols[3].button("🚑 W", key=f"ev_w_{sim_type}"): sim.inject_emergency_vehicle('west', 'ambulance')
